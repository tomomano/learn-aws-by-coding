...
AMI には，ベースとなる (空っぽの) OS を選択できるが，それに加えて，各種のプログラムがインストール済みの AMI も定義することができる．
...
https://pytorch.org/[PyTorch] は Facebook AI Research (FAIR) LAB が中心となって開発を進めている，オープンソースのディープラーニングのライブラリである．
...
実際にベンチマークを取ることでGPUとCPUの速度を比較してみよう．
